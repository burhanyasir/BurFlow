...
function applyIndustrySpecificBoosts(
  scores: IntentSignalMap,
  input: VisitorIntentEngineInput,
  path: string,
  pageType: string,
  titleText: string,
  headingText: string,
...
function getIntentScores(input: VisitorIntentEngineInput): IntentSignalMap {
  ...
  const add = (intent: VisitorIntent, points: number) => {
  ...
  if (/(\bhow much (is|does|would|will)\b|\bwhat(?:'s| is) the (price|cost)\b|\b(get|send|give) me a quote\b|\bwhat do you charge\b|\bstarting at\b)/.test(userQuestion)) {
    add('Pricing', 12);
  } else if (/(\b(price|pricing|cost|monthly|subscription|quote)\b)/.test(userQuestion)) {
    add('Pricing', 3);
  }
...
function normalizeScores(scores: IntentSignalMap): Record<VisitorIntent, number> {
  const entries = (Object.entries(scores) as Array<[VisitorIntent, number]>).map(([intent, value]) => [intent, Math.max(0, value)] as [VisitorIntent, number]);
  const total = entries.reduce((sum, [, value]) => sum + value, 0);
  const distribution = Object.fromEntries(entries.map(([intent, value]) => [intent, total > 0 ? value / total : 0])) as Record<VisitorIntent, number>;
  return distribution;
}
