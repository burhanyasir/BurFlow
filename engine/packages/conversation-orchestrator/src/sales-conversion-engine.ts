...
function deriveBusinessIntelligence(memory: ConversationMemoryData): {
  ...
  const businessType = (memory as any).businessType || (memory as any).industry || 'saas';
...
function detectVisitorIntent(message: string): { primaryIntent: string; confidence: number; supportingEvidence: string[]; recommendedNextAction: string } {
  ...
  if (/(buy|purchase|sign up|start trial|get started|free trial|ready to go|let's do it)/.test(lower)) {
    intent = 'Buying';
    confidence = 0.9;
...
function chooseRecommendedPlan(memory: ConversationMemoryData, planGoal: ConversationGoal): string {
  ...
  if (memory.companySize) {
    const sizeStr = memory.companySize.replace(/[^0-9]/g, '');
    const sizeNum = parseInt(sizeStr, 10);
    if (!isNaN(sizeNum) && sizeNum >= 1200) return 'Enterprise';
  }
...
function buildCRMAction(memory: ConversationMemoryData, planGoal: ConversationGoal): CRMIntegrationAction {
  ...
  if (memory.qualificationCollected.questionsAskedCount > 0) {
    ...
    if (!memory.qualificationCollected.completed) {
      ...
      if (!memory.companySize) missing.push('company_size');
...
function buildAnalyticsSummary(
  ...
  if (memory.companySize) {
    const sizeStr = memory.companySize.replace(/[^0-9]/g, '');
    const sizeNum = parseInt(sizeStr, 10);
    if (!isNaN(sizeNum) && sizeNum >= 1200) sizeMultiplier = 1.2;
  }
  ...
  if (plan.funnelStage === 'decision' || memory.buyingIntentDetected || (memory.companySize && /\d{4,}/.test(memory.companySize))) {
    bucket = 'hot';
