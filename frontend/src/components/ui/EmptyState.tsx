...
export function EmptyState({ icon, title, description, action, size = 'md', className }: EmptyStateProps) {
  return (
    <div className={cn('flex flex-col items-center justify-center text-center px-4', sizeStyles[size], className)}>
      {icon ? (
        <div className={cn('mb-4 text-[var(--color-neutral-300)]', iconSizes[size])}>{icon}</div>
      ) : (
        <svg className={cn('mb-4 text-[var(--color-neutral-300)]', iconSizes[size])} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {description && <p className="text-sm text-[var(--color-neutral-500)] max-w-sm mb-4">{description}</p>}
