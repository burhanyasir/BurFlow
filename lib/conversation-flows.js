...
function createFlows(knowledge) {
  ...
  function nextStepAfterService() {
    const pts = clinic.patientTypes;
    return (Array.isArray(pts) && pts.length > 0) ? "visit_type" : "date";
  }
  ...
  function detectAndHandleChange(text, state, validSteps) {
    ...
    if (entities.services.length > 0 && state.collected.service && entities.services[0] !== state.collected.service) {
      ...
      state.step = nextStepAfterService();
      return nextStepAfterService();
    }
  ...
  const flows = {
    appointment_booking: {
      ...
      steps: {
        service: {
          ...
          process(text, state) {
            ...
            const lower = text.toLowerCase();
            const aliases = (clinic.serviceAliases && typeof clinic.serviceAliases === "object") ? clinic.serviceAliases : {};
            for (const [pattern, canonical] of Object.entries(aliases)) {
              try {
                if (new RegExp(pattern, "i").test(lower)) {
                  state.collected.service = canonical;
                  state.step = nextStepAfterService();
                  return null;
                }
              } catch (e) { /* skip malformed alias pattern */ }
            }
        ...
        visit_type: {
          prompt(text, state) {
            const ptq = (clinic.patientTypeQuestion) || `Are you a new ${((clinic.customerLabel || "Customer")).toLowerCase()} or an existing ${((clinic.customerLabel || "Customer")).toLowerCase()}?`;
            return `Great, ${state.collected.service}! ${ptq}`;
          },
          process(text, state) {
        ...
        date: {
          prompt(text, state) {
            const hrs = (clinic.hours) || "our regular business hours";
            return `What day would work best for you? Our hours are ${hrs}.`;
          },
          process(text, state) {
        ...
        confirm_name: {
          ...
          process(text, state) {
            ...
            state.collected.name = state.collected.pendingName || ("Valued " + (clinic.customerLabel || "Customer"));
            delete state.collected.pendingName;
            state.step = "phone";
            state.retries.name = 0;
            return null;
      ...
      generateConfirmation(state) {
        if (state.collected._recoveryToClinic) {
          return "No problem at all! You can reach " + clinic.name + " directly at " + clinic.phone + ". Our team will be happy to help you. Is there anything else I can help with?";
        }
    ...
    lead_capture: {
      ...
      generateConfirmation(state) {
        if (state.collected._recoveryToClinic) {
          return "No problem! You can reach " + clinic.name + " directly at " + clinic.phone + ". A team member will assist you. Is there anything else I can help with?";
        }
    ...
    pricing: {
      ...
      generateConfirmation(state) {
        ...
        if (svc) {
          return `For ${svc.name}, our price is ${svc.price} and the appointment typically takes ${svc.duration}. We also offer payment plans through ${(clinic.paymentProvider) || "a third-party provider"} if needed.\n\nWould you like to book an appointment for this?`;
        }
