...
function createConversationManager(demoData) {
  ...
  function processMessage(sessionId, message) {
    ...
    if (route === "greeting" && session.history.length <= 2) {
    ...
    if (activeWf) {
      ...
      if ((route === "emergency" || route === "insurance" || route === "pricing") && route !== activeWf) {
        if (route === "pricing" && session.memory.completedPricing) {
          const reply = advanceWorkflow(session, text);
          session.history.push({ role: "bot", text: reply, timestamp: new Date().toISOString() });
          return reply;
        }
        if (route === "insurance" && session.memory.completedInsurance) {
          const reply = advanceWorkflow(session, text);
          session.history.push({ role: "bot", text: reply, timestamp: new Date().toISOString() });
          return reply;
        }
        ...
        return startAndLog(session, sessionId, route, text);
      }

      if (text.length > 4 && !isRelevantToWorkflow(text, activeWf, ws.step)) {
        if (route === "general" || route === "business_info" || intent === activeWf) {
          pauseActiveWorkflow(session);
          const answer = route === "business_info" ? handleBusinessInfo(session, text) : handleGeneralQuestion(session, text);
    ...
    if (route === "emergency" && classification.confidence >= 0.15) {
      return startAndLog(session, sessionId, "emergency", text);
    }
    if (route === "booking" && classification.confidence >= 0.15) {
      pauseActiveWorkflow(session);
      const reply = startWorkflow(session, sessionId, "appointment_booking", text);
      session.history.push({ role: "bot", text: reply, timestamp: new Date().toISOString() });
      return reply;
    }
    if (route === "insurance" && classification.confidence >= 0.15) {
      return startAndLog(session, sessionId, "insurance", text);
    }
    if (route === "pricing" && classification.confidence >= 0.15) {
      return startAndLog(session, sessionId, "pricing", text);
    }
    if (route === "lead" && classification.confidence >= 0.15) {
      return startAndLog(session, sessionId, "lead_capture", text);
    }
    if (route === "business_info" && classification.confidence >= 0.15) {
    ...
    if (route === "unknown") {
      const reply = formatUnknownReply(session, classification);
      trackResponse(session, reply);
      session.history.push({ role: "bot", text: reply, timestamp: new Date().toISOString() });
      return reply;
    }

    const base = handleGeneralQuestion(session, text);
    const reply = appendSuggestions(base, classification);
    trackResponse(session, reply);
    session.history.push({ role: "bot", text: reply, timestamp: new Date().toISOString() });
    return reply;
  }
  ...
  function handleGreeting(session) {
    ...
    const biz = knowledge.getClinicInfo().name || "our business";
    return name
      ? `Welcome back, ${name}! How can I help you today?`
      : `Hello! Welcome to ${biz}. I'm your virtual assistant. I can help you book appointments, check pricing, verify insurance, handle emergencies, or answer any questions. What can I help you with?`;
  }
  ...
  function isGenericCatchAll(response) {
    ...
    return r.includes("i'm here to help with anything related to") ||
           r.includes("i can assist with appointments, pricing, insurance") ||
           r.includes("how can i help you?");
  }

  function appendSuggestions(base, classification) {
    const suggestions = classification && classification.suggestions;
    if (!Array.isArray(suggestions) || suggestions.length === 0) return base;
    const phrase = suggestions.map(s => s.text).join(", ");
    return `${base} Did you mean: ${phrase}?`;
  }

  function formatUnknownReply(session, classification) {
    const biz = knowledge.getClinicInfo().name || "our business";
    const base = `I'm not quite sure what you're looking for, but I'm here to help with anything related to ${biz}! `;
    const suggestions = classification && classification.suggestions;
    if (Array.isArray(suggestions) && suggestions.length > 0) {
      const phrase = suggestions.map(s => s.text).join(", ");
      return `${base}Did you mean: ${phrase}?`;
    }
    return `${base}What can I help you with?`;
  }
  ...
  function handleGeneralQuestion(session, text) {
    ...
    if (/(?:call|phone|connect|speak|transfer)\s*(?:clinic|front.?desk|office|human|agent|person|staff|team|directly)/i.test(lower) ||
      ...
      mem.escalated = true;
      return `Of course! You can reach ${knowledge.getClinicInfo().name} directly at ${knowledge.getLocation().phone}. Our team will be happy to assist you. Is there anything else I can help with?`;
    }
    ...
    if (isConfused(text)) {
      ...
      if (mem.confusedCount === 2) {
        return `Let me try explaining differently. I'm a virtual assistant for ${knowledge.getClinicInfo().name}. I can help you book appointments, check our prices, verify insurance, or answer questions about our services. What would you like help with? Just say something like "book a cleaning" or "how much is invisalign?"`;
      }
    }
    ...
    if (/(?:are you|how good|reputation|review|rating|recommend|trust|experienced|testimonial|referral)\s*(?:good|reliable|trustworthy|reputable|experienced|recommended|rated)?/i.test(lower) ||
      ...
      const info = knowledge.getClinicInfo();
      const staff = Array.isArray(info.staff) ? info.staff : [];
      const staffPhrase = staff.map(s => {
        const detail = s.experience || s.specialty || s.title || "";
        return detail ? `${s.name} (${detail})` : s.name;
      }).join(", ");
      const rating = info.rating || "top";
      return `Our team is led by ${staffPhrase}. We maintain a ${rating}-star rating across review platforms, and many of our ${info.clientNoun || "patients"} come through referrals. The best way to judge is to visit us — would you like to book a consultation to meet the team?`;
    }
    ...
    if (/who are you|what are you/i.test(lower)) {
      const info = knowledge.getClinicInfo();
      const assistant = info.assistantName || "your virtual assistant";
      return `I'm ${assistant}, your virtual assistant for ${info.name}. I can help with appointments, pricing, insurance questions, emergencies, and general information about ${info.name}. How can I help you?`;
    }

    if (/pediatric|child|kid|children/i.test(lower)) {
      trackResponseType("pediatric");
      const info = knowledge.getClinicInfo();
      const pedStaff = (Array.isArray(info.staff) ? info.staff : []).find(s => (s.specialty || "").toLowerCase().includes("pediatric"));
      ...
      return `Yes, we see children of all ages!${pedStaff ? " " + pedStaff.name + " specializes in " + (info.pediatricServiceLabel || "pediatric care") + "." : ""} First visits are gentle and fun. We recommend bringing children by age 1. Would you like to schedule an appointment for your child?`;
    }

    if (/implant/i.test(lower)) {
      const info = knowledge.getClinicInfo();
      const lead = (Array.isArray(info.staff) && info.staff[0]) ? info.staff[0].name : "our team";
      const provider = info.paymentProvider || "a third-party provider";
      return `We offer implants ranging from $3,000-$4,500 per tooth. The process takes a few months but results are permanent and natural-looking. We offer ${provider} financing. Would you like to book a consultation with ${lead}?`;
    }

    if (/payment plan|financing|care ?credit/i.test(lower)) {
      const provider = knowledge.getClinicInfo().paymentProvider || "a third-party provider";
      return `We offer flexible payment plans through ${provider} with 0% financing options for 6-12 months. We also accept cash, credit cards, and HSA/FSA cards. Would you like to discuss this further?`;
    }
    ...
    if (/service|what do you offer/i.test(lower)) {
      ...
      return `Here's what we offer at ${knowledge.getClinicInfo().name}:\n${list}\n\nWhich service are you interested in?`;
    }
    ...
    return `I'm here to help with anything related to ${knowledge.getClinicInfo().name}! I can assist with appointments, pricing, insurance, emergencies, or answer general questions. What can I help you with?`;
  }
