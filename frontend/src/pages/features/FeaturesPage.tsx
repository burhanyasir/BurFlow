...
const features = [
  ...
  {
    title: 'Multi-Tenant Workspace Control',
    description: 'Manage multiple client environments or brand portfolios from a unified dashboard with granular team roles and access controls.',
    icon: 'layers'
  },
  {
    title: 'Real-Time Analytics & Operational Insights',
    description: 'Track conversation volume, token usage, user query trends, and knowledge coverage in real time.',
    icon: 'chart'
  },
  {
    title: 'Data Sovereignty & Security',
    description: 'Your data remains entirely yours. Enterprise-grade isolation ensures customer information is never trained into public models.',
    icon: 'shield'
  }
];
...
const icons: Record<string, JSX.Element> = {
  brain: (
    <svg className="h-8 w-8 text-[#5865F2]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {features.map((f, i) => (
          ...
          >
            <div className="w-12 h-12 rounded-lg bg-[#E8EAFF] flex items-center justify-center mb-4">
              {icons[f.icon]}
            </div>
