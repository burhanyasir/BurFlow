...
const LOW_CONFIDENCE = (TAXONOMY.thresholds && TAXONOMY.thresholds.lowConfidence) || 0.28;
...
function createConversationRecorder() {
  ...
  function recordTurn(sessionId, cm, inputText, replyText, intent, confidence, extra) {
    ...
    const turn = {
      ...
      lowConfidence: typeof confidence === "number" && confidence < LOW_CONFIDENCE,
      wasFallback: isGenericReply(replyText) || intent === "unknown" || intent === "clarify" ||
  ...
  function computeAnalytics() {
    ...
    let fallbackCount = 0;
    let unknownIntentTurns = 0;
    let totalIntents = 0;
    for (const conv of all) {
      for (const turn of conv.turns) {
        if (turn.intent) totalIntents++;
        if (turn.wasFallback) fallbackCount++;
        if (turn.wasFallback || turn.intent === "unknown" || turn.intent === "clarify" || turn.lowConfidence) {
          unknownIntentTurns++;
        }
      }
    }
    ...
    let abandoned = 0;
    const successOutcomes = new Set(["completed", "appointment_booked", "lead_captured", "emergency_booked"]);
    for (const conv of all) {
      const startedWorkflow = conv.turns.some(t => t.activeWorkflow);
      const completed = conv.turns.some(t => (t.completions || []).length > 0) || successOutcomes.has(conv.outcome);
      if (startedWorkflow && !completed) abandoned++;
    }
    ...
    return {
      ...
      fallbackCount,
      fallbackRate: totalTurns > 0 ? ((fallbackCount / totalTurns) * 100).toFixed(1) + "%" : "0%",
      unknownIntentTurns,
      unknownIntentRate: totalIntents > 0 ? ((unknownIntentTurns / totalIntents) * 100).toFixed(1) + "%" : "0%",
      abandonedConversations: abandoned,
      abandonmentRate: all.length > 0 ? ((abandoned / all.length) * 100).toFixed(1) + "%" : "0%"
    };
  }
